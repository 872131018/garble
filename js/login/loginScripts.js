function checkLogin()
{
	$.post('/garble/php/login/loginDriver.php', {'action': 'login',
		...
		function(response)
		{
			...
			{
				case 'loginSuccess':
					window.location = '/garble/html/homepage.html';
...
function addUser()
{
	$.post('/garble/php/login/loginDriver.php', {'action': 'addUser',
										 	'username': $('#username').val(),
										  	'password': $('#password').val()}, 
		function(response)
		{
			...
			//@TODO check for errors if response fails
		});
}